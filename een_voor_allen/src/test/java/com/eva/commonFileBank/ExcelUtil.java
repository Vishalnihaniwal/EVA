...
public class ExcelUtil {
	...
	public Workbook getWorkBook(String excelPathWithName) throws IOException {
		...
		String nameOfFile = new File(excelPathWithName).getName();
		if ((nameOfFile.substring(nameOfFile.lastIndexOf("."))).equalsIgnoreCase(".xlsx")) {
...
//			System.out.println("xlsx object created");
		} else if ((nameOfFile.substring(nameOfFile.lastIndexOf("."))).equalsIgnoreCase(".xls")) {
			xlWorkBook = new HSSFWorkbook(new FileInputStream(excelPathWithName));
//			System.out.println("xls object created");
		}
		else
		{
//			throws statement should be implemented
		}
		return xlWorkBook;
	...
	public List<Row> filterRow(String fileNameWithPath, String sheetName, String columnNameToFilter, String criteria)throws IOException {
		List<Row> row = getRow(fileNameWithPath, sheetName);
		return row.stream().filter(currentRow -> getCellData(currentRow, columnNameToFilter).equals(criteria))
				.collect(Collectors.toList());
	}

	public List<Row> filterRow(String sheetName, String columnNameToFilter, String criteria) throws IOException {
		List<Row> row = getRow(sheetName);
		return row.stream().filter(currentRow -> getCellData(currentRow, columnNameToFilter).equals(criteria))
				.collect(Collectors.toList());
	}
